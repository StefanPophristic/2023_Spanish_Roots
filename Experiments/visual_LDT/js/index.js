...
// ISI set inside the function

// Parameter to automatically change number of minutes in study instructions
var studyDuration =  "7"
/*
The total number of trials needs to be divisible by 4. Meaning that the total number
of trial pairs needs to be divisible by 2. As I currently have it, I have 93 trials
for my conditions 4 and 5, so I add an extra 3 dummy distractor trials manually.

If we are running with a different number of interveners, this math needs to be double checked.
*/
intervener_param = 1 //number of interveners between prime and target, ADD MATH CONDITION TO DESCRIPTION
...
function make_stims_per_condition() {
  ...
  */
  ...
  if (group_condition == 1) {
    ...
    stem_a_array  = [5, 10, 15, 20, 25, 30, 35, 40];
  ...
  // Add extra stimuli to 93 length arrays in order to be able to have the
  // ultimate number of trials be divisible by 4 (in order to have )
  if (all_stims.length == 93) {
    all_stims.push({id: 43,
                    prime: "bomo",
                    target: "roga",
                    condition: "distractor"});
    all_stims.push({id: 44,
                    prime: "bapo",
                    target: "volo",
                    condition: "distractor"});
    all_stims.push({id: 45,
                    prime: "praca",
                    target: "stado",
                    condition: "distractor"});
  }
...
function make_slides(f) {
  ...
  slides.objecttrial = slide({
    ...
    present_handle : function(stim) {
      ...
      $("#target_word").hide();
      $("#fixation_cross").hide(); // was .show() for fixation cross
    ...
    /*
    Take in the response value which is either 102 (f) or 106 (j)
    ...
    */
      ...
      // // Wait the stimCrossTime before continuing to the next fixation cross
      // setTimeout(() => {
      //   // reseet flag after trial finishes
      //   this.processingKey = false;
      //
      ...
      // I think this is for when I have breaks, but I'm not sure, needs to be tested
...
function init() {
  ...
  group_condition = getParameterByName('group');

  console.log(group_condition);
  ...
  exp.all_stims = interweaveItems(all_stims, intervener_param);
...
// "objecttrial1", "break1",
  ...
  exp.nQs = (exp.all_stims.length + 8);

  // Total number of trials in order to have the progress bar work
  // + 8 for all other trial slides including instructions, practics, and questionaire
